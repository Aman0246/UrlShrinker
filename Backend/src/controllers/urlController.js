...
const shorten = async function (req, res) {
  try {
    ...
    if (cacheUrl) {
      ...
      return res.status(200).send({ status: true, message: "Already available in caches", data:data });
    }
    ...
    if (url) {
      ...
      return res.status(200).send({ status: true, message: "Already available in db Now in caches",data:data });
    }
